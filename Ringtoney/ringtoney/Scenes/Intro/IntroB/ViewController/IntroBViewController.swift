...
class IntroBViewController: ViewController {
    
    //MARK:- Property
    var backgroundImageView = ImageView().then {
        $0.image = R.image.background_introB()
    ...
    var descLabel = Label().then {
        $0.text = "Search in catalog or make your own ringtones"
        $0.font = FontsManager.nasalization.font(size: 23)
        $0.numberOfLines = 0
        $0.textColor = UIColor.init(hexString: "FFFFFF")
    ...
    override func bindViewModel() {
        ...
        guard let viewModel = viewModel as? IntroBViewModel else { return }
        
        //Input
        let inputs = IntroBViewModel.Input(
...
//MARK:- Extensition
extension IntroBViewController {
