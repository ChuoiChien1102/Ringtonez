...
final class TrimManager : NSObject {
    ...
    class func trim( url: URL,
        ...
        if !FCFileManager.existsItem(atPath: ".tmp/tmp.m4r") {
            ...
            print("Input: start \(start) - duration: \(duration) ")
            print("Input: speed \(speed)")
            print("Input: fadeIn \(fadeIn) - fadeOut: \(fadeOut) ")
...
//                TrimManager.rateSpeed(audioAsset: asset, audioAssetSourceTrack: audioAssetSourceTrack, audioCompositionTrack: audioCompositionTrack!, speed: Double(speed))
    ...
    class func fadeInfadeOut(turnOnFadeIn:Bool,
        ...
        if trimTime > 9.0 {
            ...
            let fadeInEnd = CMTime.init(seconds: 4.0, preferredTimescale: 1)
          let fadeOutStart = CMTime.init(seconds: trimTime, preferredTimescale: 1) - (CMTime.init(seconds: 5.0, preferredTimescale: 1))
